    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (trimmed.startsWith('[') && trimmed.endsWith(']')) {
            ...
            if (eqIndex !== -1) {
                ...
                currentSection.properties.push({ id: generateId(), type: 'property', key, value: val, raw: rawLine, hasEquals: true });
            } else {
                currentSection.properties.push({ id: generateId(), type: 'property', key: rawLine.trim(), value: '', raw: rawLine, hasEquals: false });
            }
        }
    }
    ...
    for (const section of sections) {
        ...
        for (const prop of section.properties) {
            if (prop.type === 'blank') {
                ...
                result += prop.hasEquals === false ? `${prop.key}\n` : `${prop.key}=${prop.value}\n`;
            }
        }
    }
...
export const ConfigEditorPage: React.FC<ConfigEditorPageProps> = ({ projectPath, projectName, onBack }) => {
    ...
    const confirmDiscardChanges = () => {
        ...
        return window.confirm(t('config.unsavedChanges') || 'You have unsaved changes. Discard them?');
    };

    const handleTabChange = (tab: 'visual' | 'plugins' | string) => {
                                                        ...
                                                        <div className="p-2 space-y-1">
                                                            ...
                                                            <div className="pt-2 px-3">
                                                                <button
                                                                    onClick={() => {
                                                                        const p = [...parsedIni];
                                                                        p[sIndex].properties.push({ id: generateId(), type: 'property', key: '', value: '', hasEquals: true });
                                                                        setParsedIni(p);
                                                                        handleStructuredChange();
                                                                    }}
