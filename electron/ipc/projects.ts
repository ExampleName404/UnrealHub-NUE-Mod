...
async function saveManualProject(projectPath: string): Promise<boolean> {
    ...
    const fileName = path.basename(projectPath, '.uproject');
    let version = 'Unknown';

    try {
        const content = JSON.parse(await fs.readFile(projectPath, 'utf-8'));
        if (content.EngineAssociation) {
            version = content.EngineAssociation;
        }
    } catch (error) {
        console.warn(`Failed to parse ${projectPath}:`, error);
    }
    ...
    if (excluded.includes(projectPath)) {
        await writeJsonFile(EXCLUDED_PATH, excluded.filter(pathItem => pathItem !== projectPath));
    }

    const config = await loadConfig();
    if (config.projectPaths.includes(projectDir)) {
        config.projectPaths = config.projectPaths.filter(pathItem => pathItem !== projectDir);
...
export function registerProjectHandlers() {
    ...
    ipcMain.handle('add-project-file', async () => {
        ...
        return saveManualProject(filePath);
    });
    ...
    ipcMain.handle('launch-project', async (_, projectPath: string, args?: string) => {
        ...
        try {
            ...
            if (engineExe) {
                ...
                const spawnArgs = [projectPath, ...parseLaunchArgs(args)];
