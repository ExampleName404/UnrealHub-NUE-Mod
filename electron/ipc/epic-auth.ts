...
async function getStoredClientSecret(): Promise<string | null> {
    try {
        const keytar = await import('keytar');
        const secret = await (keytar as any).getPassword(SERVICE_NAME, ACCOUNT_NAME);
        return secret || null;
    } catch (e) {
        return null;
    }
...
async function setStoredClientSecret(secret: string): Promise<boolean> {
    try {
        const keytar = await import('keytar');
        await (keytar as any).setPassword(SERVICE_NAME, ACCOUNT_NAME, secret);
        return true;
    } catch (e) {
        return false;
    }
...
async function clearStoredClientSecret(): Promise<boolean> {
    try {
        const keytar = await import('keytar');
        return await (keytar as any).deletePassword(SERVICE_NAME, ACCOUNT_NAME);
    } catch (e) {
        return false;
    }
}
...
type EpicLoginResult = {
    token: EpicToken | null;
    error?: string;
};

let currentToken: EpicToken | null = null;
let activeLoginFlow: Promise<EpicLoginResult> | null = null;
...
function saveToken(token: EpicToken) {
    ...
    try {
        ...
        if (safeStorage.isEncryptionAvailable()) {
            ...
            const payload: EncryptedTokenFile = { v: 1, encrypted: true, data: encrypted };
...
function loadToken(): EpicToken | null {
    ...
    try {
        ...
        if (fs.existsSync(tokenPath)) {
            ...
            // Preferred path: JSON wrapper that contains encrypted token payload.
            try {
                const parsed = JSON.parse(raw.toString('utf-8')) as EpicToken | EncryptedTokenFile;
                if (
                    parsed &&
                    typeof parsed === 'object' &&
                    (parsed as EncryptedTokenFile).encrypted === true &&
                    typeof (parsed as EncryptedTokenFile).data === 'string'
                ) {
                    if (!safeStorage.isEncryptionAvailable()) return null;
                    const decryptedJson = safeStorage.decryptString(
                        Buffer.from((parsed as EncryptedTokenFile).data, 'base64'),
                    );
                    const token = JSON.parse(decryptedJson) as EpicToken;
                    currentToken = token;
                    return token;
                }

                currentToken = parsed as EpicToken;
                return currentToken;
            } catch {
                // Migration/fallback: older plaintext JSON or legacy raw encrypted bytes.
                if (safeStorage.isEncryptionAvailable()) {
                    try {
                        const decryptedJson = safeStorage.decryptString(raw);
                        const token = JSON.parse(decryptedJson) as EpicToken;
                        currentToken = token;
                        return token;
                    } catch {
                ...
                const token = JSON.parse(raw.toString('utf-8')) as EpicToken;
                currentToken = token;
                return token;
            }
        }
...
async function exchangeCode(code: string): Promise<EpicToken> {
    const res = await epicRequest({
        ...
        method: 'POST',
        headers: {
            'Authorization': `Basic ${await getBasicAuth()}`,
        },
...
async function getValidToken(promptOnMissing = false): Promise<string | null> {
    let token = loadToken();
    if (!token) {
        if (!promptOnMissing) return null;
        const login = await startEpicLoginFlow();
        return login.token?.access_token || null;
    }
...
async function startEpicLoginFlow(): Promise<EpicLoginResult> {
    if (activeLoginFlow) return activeLoginFlow;
    ...
    activeLoginFlow = new Promise<EpicLoginResult>((resolve) => {
        ...
        const handleCode = async (code: string) => {
            ...
            try {
                ...
                finish({ token: null, error: e.message });
            }
        };
...
export function registerEpicAuthHandlers() {
    ...
    ipcMain.handle('epic-auth-login', async () => {
        const result = await startEpicLoginFlow();
        return result.token
            ? { success: true, displayName: result.token.displayName }
            : { success: false, error: result.error || 'Login cancelled' };
    });
    ...
    // Store/clear client secret in OS credential store via keytar
    ipcMain.handle('epic-store-client-secret', async (_evt, secret: string) => {
        if (!secret) return false;
        return await setStoredClientSecret(secret);
    });

    ipcMain.handle('epic-clear-client-secret', async () => {
        return await clearStoredClientSecret();
    });
